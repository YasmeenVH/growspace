...
class GrowSpaceEnvSpotlightMnist(gym.Env):
    ...
    def tree_grow(self, activated_photons, mindist, maxdist):
        ...
        for branch in branches_trimmed:
            if branch.grow_count > 0:
                x2 = branch.x2 + branch.grow_direction[0] / branch.grow_count
                y2 = branch.y2 + branch.grow_direction[1] / branch.grow_count
    ...
    def get_observation(self, debug_show_scatter=False):
        ...
        cv2.circle(
            img, tuple(self.to_image(self.focus_point)), int(self.focus_radius * self.height), (0, 255, 0), thickness=2
        )
