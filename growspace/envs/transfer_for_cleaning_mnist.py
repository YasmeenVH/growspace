...
class GrowSpaceEnv_Mnist(gym.Env):
    ...
    def __init__(self, width=DEFAULT_RES, height=DEFAULT_RES, obs_type = None, level=None):
    ...
    def light_move_R(self):
        if np.around(self.x1_light, 2) >= 1-LIGHT_WIDTH:  # limit of coordinates
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for _ in range(50):
            ...
            b,t,c,f = gse.step(action)
            print(f["new_branches"])         cv2.waitKey(1)  # this is necessary or the window closes immediately
        else:
            return img
